...
Passive Music Listening without any Biofeedback system has mixed results. To covnert the heartbeat to music, a process called mapping is necessary that takes the heart beat as an input and outputs a song. There are two forms of structural mapping processes: Continuous Mapping and discrete mapping. During the continuous mapping the volume, the BPM are directly and constantly modulated by the incoming rPPG data stream. This provides the user with an immediate, "analog" reflection of their physiological state, which is essential for developing fine-tuned accuracy. On the other hand discrete mapping only changes the song or the frequency when a specific threshold is reached. This strategy is particularly effective for signaling long-term state transitions or poviding clear rewards for successful relxation.

The Biofeedback Music has certain functional roles. It acts as a monitor so that the user can perceive internal processes, that are otherwise hidden from the conscious mind. It also acts as a guide because it pulls the user to a specific desired physiological state. Additionally, it acts as a reinforce as it increases the users' motivation with the process over extended periods \cite{bergstrom2014using}.
