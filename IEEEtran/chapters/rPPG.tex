...
The face was divided into several small squares forming a grid layout. For every part of the grid the signal-to-noise ratio was calculated. The cheek regions were identified to have the smallest amount of noises and are therefore choosen for the pulse extraction \cite{lempe2013roi}.
...
To separate the actual signal from the noise, a matrix $W$ has to be found, that solves the equation $\hat{s}(t) = Wx(t)$. To find the matrix, it must be searched for statistical independence. The Java algorithm  (Joint Approximate Diagonalization of Eigenmatrices) is employed for this. JADE is an iterative approach that find the best solution by trial and error. Consequently, this leads to an increase in computational load and latency (Figure \ref{fig:color_channels}) \cite{poh2010non}.
